...
if($_SERVER['REQUEST_METHOD'] == "POST" && isset($_POST['add_equipment']) && !$invalid){
  ...
  if(isset($updated) && !empty($updated)){
    ...
    $stmt = $conn->prepare('INSERT INTO '.$table.' ('.$updated.',budget_id) VALUES ('.$columns.',?)');
    $format = $format."s";
    $values[] = $budget_id;
...
<html>
  <head>
    ...
    <title>Add Large Equipment</title>
  ...
  <body>
    ...
    <div class="content">
      ...
      if(!$invalid){
        ...
        echo '<div>';
          echo '<button type="submit" name="add_equipment" class="submit-button">Add</button>';
        echo '</div>';
        echo '</form>';
